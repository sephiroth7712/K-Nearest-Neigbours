...
int main(int argc, char *argv[])
{
    ...
    // ensuring that at max we create 256 threads per process
    int num_threads = mpi_num_processes >= 4 ? 1024 / mpi_num_processes : 256;
    ...
    if (mpi_rank == 0)
    {
        ...
        printf("The %i-NN classifier for %lu test instances and %lu train instances required %llu ms CPU time for MPI with %d processes and %d threads. Accuracy was %.2f\%\n", k, test->num_instances(), train->num_instances(), (long long unsigned int)time_difference, accuracy, mpi_num_processes, num_threads);
