...
// Run test on huff as "salloc --partition GPU --qos gpu --nodes 2 --ntasks-per-node 1 --cpus-per-task 1 mpirun -np 2 ./multi_gpu datasets/large-train.arff datasets/large-test.arff 3 2"
...
int *KNN(ArffData *train, float *train_matrix, ArffData *test, float *test_matrix, int k, int mpi_rank, int mpi_num_processes)
{
    ...
    int *minimum_indexes = (int *)calloc(partial_test_num_instances[mpi_rank] * k, sizeof(int));
    ...
    cudaMalloc(&d_test, partial_test_num_instances[mpi_rank] * num_attributes * sizeof(float));
    cudaMalloc(&d_distances, partial_test_num_instances[mpi_rank] * train_num_instances * sizeof(float));
    cudaMalloc(&d_predictions, partial_test_num_instances[mpi_rank] * sizeof(int));
    cudaMalloc(&d_minimum_indexes, partial_test_num_instances[mpi_rank] * k * sizeof(int));
    ...
    cudaMemcpy(d_test, test_matrix + displs[mpi_rank] * num_attributes, partial_test_num_instances[mpi_rank] * num_attributes * sizeof(float), cudaMemcpyHostToDevice);
    ...
    int gridDimSizeY = (partial_test_num_instances[mpi_rank] + threadsPerBlockDim - 1) / threadsPerBlockDim;
    dim3 blockSize(threadsPerBlockDim, threadsPerBlockDim);
    dim3 gridSize(gridDimSizeX, gridDimSizeY);

    // Calculate the distance matrix
    distance<<<gridSize, blockSize, 2 * threadsPerBlockDim * num_attributes * sizeof(float)>>>(d_train, d_test, d_distances, train_num_instances, partial_test_num_instances[mpi_rank], num_attributes);

    threadsPerBlockDim = 128;
    dim3 blockSize2(threadsPerBlockDim);
    gridDimSizeX = 1;
    gridDimSizeY = partial_test_num_instances[mpi_rank];
    ...
    cudaMemcpy(minimum_indexes, d_minimum_indexes, k * partial_test_num_instances[mpi_rank] * sizeof(int), cudaMemcpyDeviceToHost);

    // Derive the class of the test element using its k-minimum distances
    for (int queryIndex = 0; queryIndex < partial_test_num_instances[mpi_rank]; queryIndex++)
    {
        for (int j = 0; j < k; j++)
        {
        ...
        partial_predictions[queryIndex] = max_class;
    ...
    /**************************************************************/
    /*** WRITE THE CODE OF THE MEMORY COPY FROM GPU TO CPU HERE ***/
    /**************************************************************/
    ...
    /***********************************************************/
    /*** WRITE THE CODE TO GATHER PREDICTIONS USING MPI HERE ***/
    /***********************************************************/
    ...
    /**********************************************/
    free(classCounts);
    free(partial_predictions);
    free(partial_test_num_instances);
...
int main(int argc, char *argv[])
{
    if (argc != 5)
    {
        printf("Usage: ./program datasets/train.arff datasets/test.arff k nodes");
        exit(0);
    }

    // k value for the k-nearest neighbors
    int k = strtol(argv[3], NULL, 10);
    int nodes = strtol(argv[4], NULL, 10);
    ...
    predictions = KNN(train, train_matrix, test, test_matrix, k, mpi_rank, mpi_num_processes);
    ...
    if (mpi_rank == 0)
    {
        ...
        printf("Multi GPU: The %i-NN classifier for %lu test instances and %lu train instances required %f ms total time for MPI with %d processes on %d nodes. Kernel and memcpy required %f ms. Kernel only required %f ms. Accuracy was %.2f%\n", k, test->num_instances(), train->num_instances(), time_difference, mpi_num_processes, nodes, kernel_memcpy_runtime, kernel_runtime, accuracy);
